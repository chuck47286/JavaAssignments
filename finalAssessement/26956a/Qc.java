...
public class Qc extends Application{
     ...
     *   @param f The group to which the x-axis is to be added.
     */
    public static Line functionToLine(Function<Double,Double> f,double angle) {
    	Line line = new Line(); 
    	line.setStartX(X_SIZE); 
    	line.setStartY(Y_SIZE); 
    	double x=X_SIZE-X_SIZE*Math.cos(angle*Math.PI/180);
    	double y=Y_SIZE-Y_SIZE*Math.sin(angle*Math.PI/180);
    	line.setEndX(x); 
    	line.setEndY(y); 	
		return line;
    ...
    public void start(Stage stage) throws Exception {
        ...
        Group root = new Group(polyline,line);


        // The scene consists of just one group.
        Scene scene = new Scene(root, X_SIZE + D_X_SIZE,Y_SIZE + D_Y_SIZE);
    ...
    public static void setValues(Function<Double, Double> f,
                                 int numberOfValues, double left, double right,double angle){
            a = left;
            b = right;
            n = numberOfValues;
            min = min(f,n,a,b);
    ...
    public static void main(String[] args) {
        ...
        Function<Double, Double> f1 = x -> {return Math.sqrt(1-x*x);};
//        setValues(f1, 200, -20, 20,20);
        setValues(f1, 200, -1, 0,45.0);
        launch(args);
    }
}
